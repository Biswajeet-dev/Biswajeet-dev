# 👋 Hi, I'm Biswajeet

🚀 **Full Stack Developer | Backend Specialist | System Design Enthusiast**

---

## 🧠 About Me

I design and build **scalable, production-grade applications** with a strong focus on backend architecture, modular design, and clean data modeling.

* 💻 1.5+ years of development experience
* ⚙️ Specialized in **Java (Spring Boot)** and **Node.js** ecosystems
* 🧩 Passionate about **modular monolith → microservices evolution**
* 🏗️ Building an **Enterprise Operations Platform**
* ☁️ Exploring **Kubernetes-native architectures**

---

## ⚡ Tech Stack

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge\&logo=java\&logoColor=white)
![Spring](https://img.shields.io/badge/SpringBoot-6DB33F?style=for-the-badge\&logo=springboot)
![Node](https://img.shields.io/badge/Node.js-339933?style=for-the-badge\&logo=nodedotjs)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge\&logo=react)
![Postgres](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge\&logo=postgresql)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge\&logo=docker)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge\&logo=kubernetes)

---

## 📊 GitHub Stats

![Sarkar's GitHub stats](https://github-readme-stats.vercel.app/api?username=Biswajeet-dev\&show_icons=true\&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Biswajeet-dev\&layout=compact\&theme=tokyonight)

![Streak](https://streak-stats.demolab.com?user=Biswajeet-dev\&theme=tokyonight)

---

## 🧩 Featured Work

### 🏗️ Enterprise Operations Platform

* Modular Monolith → Microservices ready
* HR | Learning | Ops | Analytics | Identity
* Spring Boot 4 + Kubernetes-native design

---

### 🏥 Patient Simulation System (PatSim)

* Complex **Case–Visit–User** relationships
* Advanced JPA (Composite Keys, Join Tables)
* Real-world medical workflow modeling

---

### 🎯 Quiz & Learning Engine

* Dynamic quizsets & configurations
* Fine-grained answer/explanation control
* Scalable learning platform backend

---

## 🔥 What I Focus On

* Clean backend architecture
* Scalable system design
* Complex database modeling
* Production-ready APIs

---

## 🎯 Current Goals

* Distributed Systems Mastery
* Kubernetes Deep Dive
* Microservices at scale

---

## ⚙️ Activity Graph

![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Biswajeet-dev\&theme=tokyo-night)

---

## 📫 Connect

* 💼 Open for backend/system design collaborations
* ⚡ Always building something scalable

---

⭐ *Explore my repositories and build something impactful together.*

---

## 🧠 Dev Quote

![Quote](https://quotes-github-readme.vercel.app/api?type=horizontal\&theme=tokyonight)

---




